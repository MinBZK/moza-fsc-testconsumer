...
|-------|--------|--------|------|
| **group** | group-intermediate (`ca/`) | `out/uitvraag-org/<endpoint>/{cert,key}.pem` | Extern: mesh-mTLS, token- en contract-endpoints (hergebruikt dezelfde identity-cert). |
| **internal** | per-peer internal-CA (`internal/uitvraag-org/ca/`) | `internal/uitvraag-org/<endpoint>/{cert,key}.pem` | Intern: component-tot-component binnen de peer, los van de group-trust-anchor. |
...
>   uitvraag-org moet ketenen naar **fsc-testnet's** group-root (anders vertrouwt de directory de
...
```bash
openssl x509 -in out/uitvraag-org/outway/cert.pem -noout -subject | grep 00000000000000000020
```
...
for f in pki/peers/uitvraag-org/*/csr.json; do
  jq -e '.serialnumber=="00000000000000000020"' "$f" >/dev/null && echo "$f OK"
done
