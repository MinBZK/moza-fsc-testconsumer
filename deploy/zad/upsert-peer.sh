...
#
# Eigen project = eigen ZAD-API-key: ZAD_API_KEY hoort bij het eigen project, NIET de
# key van het bovenliggende systeem en NIET de key van de aanbiedende provider-peer.
#
...
# cloneFrom is optioneel (ZAD_PEER_CLONE_FROM) maar afgeraden.
# NIET via de API (UI-only): bijlagen (cert-mount) + "Publicatie op het web" (passthrough-TLS) —
# zie cert-manifest.md.
#
# DB: net als bij de provider-peer draaien we een EIGEN postgres-component `uvrpg` (self-hosted)
# i.p.v. ZAD's managed Postgres — die laat ons de init/schema's niet inrichten. uvrmgr/uvrctl/uvrtxlog
# krijgen een CONCRETE STORAGE_POSTGRES_DSN naar `uvrpg:5432` (in env_vars, geen ZAD $DATABASE_*-
...
IMAGE_TAG="${3:-v1.43.7}"                        # OpenFSC-versie: outway stock-image + default-tag voor de migrate-wrappers
...
# --- Self-hosted Postgres (component uvrpg) ---------------------------------------------------------
# ZAD's managed Postgres laat ons het schema/init niet inrichten (geen init-scripts, geen CREATE
# SCHEMA-rechten op eigen voorwaarden). Daarom draaien we een EIGEN postgres-component `uvrpg` die we
...
[ "${MODE}" = apply ] && : "${ZAD_PG_PASSWORD:?zet ZAD_PG_PASSWORD in je env (wachtwoord voor de self-hosted uvrpg-Postgres)}"
...
# is intern bereikbaar als `<deployment>-<component>:<poort>` (bv. `test-uvrmgr:9443`). De interne
...
UVRMGR_ENV="$(printf '%s\n' \
  ...
  "TLS_GROUP_CERT=/etc/fsc/out/uitvraag-org/manager/cert.pem" \
  "TLS_GROUP_KEY=/etc/fsc/out/uitvraag-org/manager/key.pem" \
  "TLS_GROUP_TOKEN_CERT=/etc/fsc/out/uitvraag-org/manager/cert.pem" \
  "TLS_GROUP_TOKEN_KEY=/etc/fsc/out/uitvraag-org/manager/key.pem" \
  "TLS_GROUP_CONTRACT_CERT=/etc/fsc/out/uitvraag-org/manager/cert.pem" \
  "TLS_GROUP_CONTRACT_KEY=/etc/fsc/out/uitvraag-org/manager/key.pem" \
  "TLS_ROOT_CERT=/etc/fsc/internal/uitvraag-org/ca/root.pem" \
  "TLS_CERT=/etc/fsc/internal/uitvraag-org/manager/cert.pem" \
  "TLS_KEY=/etc/fsc/internal/uitvraag-org/manager/key.pem" \
  "TLS_INTERNAL_UNAUTHENTICATED_ROOT_CERT=/etc/fsc/internal/uitvraag-org/ca/root.pem" \
  "TLS_INTERNAL_UNAUTHENTICATED_CERT=/etc/fsc/internal/uitvraag-org/manager/cert.pem" \
  "TLS_INTERNAL_UNAUTHENTICATED_KEY=/etc/fsc/internal/uitvraag-org/manager/key.pem" \
  "SELF_ADDRESS=https://${UVRMGR_HOST_DISPLAY}:443" \
...
# Sinds de self-hosted Postgres (uvrpg) is de STORAGE_POSTGRES_DSN concreet (geen ZAD $DATABASE_*-
...
UVRCTL_ENV="$(printf '%s\n' \
  ...
  "TLS_ROOT_CERT=/etc/fsc/internal/uitvraag-org/ca/root.pem" \
  "TLS_CERT=/etc/fsc/internal/uitvraag-org/controller/cert.pem" \
  "TLS_KEY=/etc/fsc/internal/uitvraag-org/controller/key.pem" \
...
UVRTXLOG_ENV="$(printf '%s\n' \
  "LOG_TYPE=live" "LOG_LEVEL=info" \
  "LISTEN_ADDRESS=0.0.0.0:8443" \
  "MONITORING_ADDRESS=0.0.0.0:8081" \
  "TLS_ROOT_CERT=/etc/fsc/internal/uitvraag-org/ca/root.pem" \
  "TLS_CERT=/etc/fsc/internal/uitvraag-org/txlog/cert.pem" \
  "TLS_KEY=/etc/fsc/internal/uitvraag-org/txlog/key.pem")"
UVRTXLOG_ALIASES=""

# --- self-hosted Postgres: component-env + concrete DSN per FSC-component -----------------------------
# uvrpg draait het officiële postgres-image (config puur via POSTGRES_*-env, geen command nodig). PGDATA
...
# Concrete DSN naar de uvrpg-Service met per-component search_path. Toegevoegd aan env_vars (niet
...
_pg_dsn() {  # $1=search_path-schema (mag leeg)
  printf 'STORAGE_POSTGRES_DSN=postgres://%s:%s@%s:5432/%s?sslmode=%s%s' \
    "${PG_USER}" "${PG_PASSWORD}" "${UVRPG_SVC}" "${PG_DB}" "${PG_SSLMODE}" "${1:+&search_path=${1}}"
}
UVRMGR_ENV="${UVRMGR_ENV}"$'\n'"$(_pg_dsn "${MGR_SCHEMA}")"
UVRCTL_ENV="${UVRCTL_ENV}"$'\n'"$(_pg_dsn "${CTL_SCHEMA}")"
UVRTXLOG_ENV="${UVRTXLOG_ENV}"$'\n'"$(_pg_dsn "${TXLOG_SCHEMA}")"
...
# listener; uvrpg alleen :5432. Geen managed-DB-binding meer ([]): de DB is nu de uvrpg-component.
# Deze arrays moeten sporen met de LISTEN_ADDRESS_*-poorten in de env-blobs hierboven.
UVRPG_BODY="$(component_body uvrpg "${POSTGRES_IMAGE}" '[5432]' "${UVRPG_ENV}" '[]' "")"
...
if [ "${MODE}" = plan ]; then
  ...
  echo "### component uvrpg (self-hosted Postgres + init-schema's)"; echo "${UVRPG_BODY}"
...
post "uvrpg"    "/components" "${UVRPG_BODY}"      # DB eerst; de FSC-componenten retryen tot hij up is
