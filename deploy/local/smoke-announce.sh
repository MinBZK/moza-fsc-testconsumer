#!/usr/bin/env bash
# Smoke: bewijst dat uitvraag-org zich aanmeldt (announce) bij de directory ÉN dat dat via de
# :443-SNI-mesh gaat. Pollt de directory-DB tot de uitvraag-org-OIN met een manager_address op
...
echo "smoke: wachten tot uitvraag-org ($CONSUMER_OIN) announce't bij de directory (op :443)..."
elapsed=0
while [ "$elapsed" -lt "$TIMEOUT" ]; do
  ...
  if printf '%s\n' "$rows" | grep -qx "$CONSUMER_OIN"; then
    echo "OK: uitvraag-org is aangemeld bij de directory (manager_address op :443)."
...
echo "FAIL: uitvraag-org ($CONSUMER_OIN) niet aangemeld op :443 binnen ${TIMEOUT}s." >&2
...
"${COMPOSE[@]}" logs --tail=50 \
  postgres manager-directory migrate-uitvraag-org manager-uitvraag-org >&2 || true
exit 1
